# UC-04 personality feature — 2026-09-07-personality-feature-2-arms-3-variants-en-cs-425-customers

Ran on: `substrate.db` (sha256 aba47a161d87…), 425 linked customers, 18213 products
When: 2026-09-07, 1 min 21 s
Model calls: none
...
## LightGBM on engineered features
    ...
    full, en: none 2, inferred 1, sampled 2                                                                    (hits in the top 10 of 425)
    full, cs: none 2, inferred 1, sampled 2                                                                    (hits in the top 10 of 425)
    sampled, en: none 151, inferred 152, sampled 151                                                           (hits in the top 10 of 425)
    sampled, cs: none 151, inferred 152, sampled 151                                                           (hits in the top 10 of 425)
    inferred vs none, en, full: 1 vs 2 (-1) on 425; only inferred 1, only none 2; sign p 1.0                   (paired on the same customers and candidate lists)
    sampled vs none, en, full: 2 vs 2 (+0) on 271; only sampled 1, only none 1; sign p 1.0                     (paired on the same customers and candidate lists)
    inferred vs sampled, en, full: 1 vs 2 (-1) on 271; only inferred 1, only sampled 2; sign p 1.0             (paired on the same customers and candidate lists)
    inferred vs none, en, sampled: 152 vs 151 (+1) on 425; only inferred 23, only none 22; sign p 1.0          (paired on the same customers and candidate lists)
    sampled vs none, en, sampled: 96 vs 99 (-3) on 271; only sampled 13, only none 16; sign p 0.711            (paired on the same customers and candidate lists)
    inferred vs sampled, en, sampled: 100 vs 96 (+4) on 271; only inferred 15, only sampled 11; sign p 0.557   (paired on the same customers and candidate lists)
    inferred vs none, cs, full: 1 vs 2 (-1) on 425; only inferred 1, only none 2; sign p 1.0                   (paired on the same customers and candidate lists)
    sampled vs none, cs, full: 2 vs 2 (+0) on 271; only sampled 1, only none 1; sign p 1.0                     (paired on the same customers and candidate lists)
    inferred vs sampled, cs, full: 1 vs 2 (-1) on 271; only inferred 1, only sampled 2; sign p 1.0             (paired on the same customers and candidate lists)
    inferred vs none, cs, sampled: 152 vs 151 (+1) on 425; only inferred 23, only none 22; sign p 1.0          (paired on the same customers and candidate lists)
    sampled vs none, cs, sampled: 96 vs 99 (-3) on 271; only sampled 13, only none 16; sign p 0.711            (paired on the same customers and candidate lists)
    inferred vs sampled, cs, sampled: 100 vs 96 (+4) on 271; only inferred 15, only sampled 11; sign p 0.557   (paired on the same customers and candidate lists)
...
The largest paired difference is +4 hits of 271 (lightgbm_features, inferred vs sampled, en, sampled); no pair passes the sign test at 5 %, so on this data a Big Five profile, inferred or sampled, adds nothing measurable to a classical recommender. LightGBM moves by a few hits between runs even with pinned threads, so differences of that size are noise; the linear SVM cannot react to a customer-level column at all (identical rankings by construction).
