# UC-04 outputs for UC-01 — 2026-09-07-outputs-for-uc01-agy-gemini-3.8-flash-medium-2-contacts
...
When: 2026-09-07, 40 s, 10 model calls
...
## Recommendation reasons (ALS top-k + one Czech sentence each)
    ...
    20.0 words median, 24 max                          (the rule says at most 30)
    12.3 s median per call                             (the call itself)
...
## Aspects (from the Czech reviews, verbatim quotes)
    ...
    7 of 7 quotes found verbatim                       (whitespace and case aside; a paraphrase counts as not grounded)

## Classical fields

    topics for 425 contacts                            (LDA, 30 topics over the catalogue titles, top 5 per contact; lifecycle from the substrate's rule)

6 of 6 recommendation sentences cite only purchases the customer made and 7 of 7 aspect quotes are verbatim in the reviews; these are the checks the thesis reports, because nobody receives these texts and nothing else about them can be measured here. Whether they help a message is judged in UC-01, where the texts are read.
